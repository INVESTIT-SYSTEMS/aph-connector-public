...
document.addEventListener('DOMContentLoaded', function () {
    ...
    let titleTextContainer = document.getElementById('title-text-container');
    if(titleInput != null)
    {
        let textLength = titleInput.length;
        titleTextContainer.innerText = titleInput.value.length  + ' / ' + titleInput.maxLength;
        titleInput.addEventListener('input', (e) => {
            console.log(e.target.value);
            textLength = e.target.value.length;
            titleTextContainer.innerText = titleInput.maxLength === -1 ? textLength : textLength  + ' / ' + titleInput.maxLength;
            if(textLength > titleInput.maxLength && titleInput.maxLength !== -1){
                titleTextContainer.style.color = 'red';
            }
            else{
                titleTextContainer.style.color = 'black'
            }
        });
    }
});
